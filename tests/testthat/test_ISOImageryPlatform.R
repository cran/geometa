# test_ISOImageryPlatform.R
# Author: Emmanuel Blondel <emmanuel.blondel1@gmail.com>
#
# Description: Unit tests for ISOImageryPlatform.R
#=======================
require(geometa, quietly = TRUE)
require(testthat)

context("ISOImageryPlatform")
...
test_that("encoding",{
  ...
  instr <- ISOImageryInstrument$new()
  instr$setIdentifier("identifier")
  instr$setType("type")
  instr$setDescription("description")
  md$addInstrument(instr)
  
  expect_is(md, "ISOImageryPlatform")
  xml <- md$encode()
  expect_is(xml, "XMLInternalNode")
  
  #decoding
  md2 <- ISOImageryPlatform$new(xml = xml)
  xml2 <- md2$encode()
