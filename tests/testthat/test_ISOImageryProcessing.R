# test_ISOImageryProcessing.R
# Author: Emmanuel Blondel <emmanuel.blondel1@gmail.com>
#
# Description: Unit tests for ISOImageryProcessing.R
#=======================
require(geometa, quietly = TRUE)
require(testthat)

context("ISOImageryProcessing")
...
test_that("encoding",{
  ...
  md$setProcedureDescription("description")
  md$addSoftwareReference(ct)
  md$addDocumentation(ct)
  md$setRunTimeParameters("parameters")
  
  expect_is(md, "ISOImageryProcessing")
  xml <- md$encode()
  expect_is(xml, "XMLInternalNode")
  
  #decoding
  md2 <- ISOImageryProcessing$new(xml = xml)
  xml2 <- md2$encode()
